...
<section class="home">
 ...
 FROM previsions  
 WHERE previsions.mois LIKE'%$mois%' AND previsions.centre=4760 AND   previsions.parametre=1 AND previsions.anne='$ann' ";
 ...
 FROM realisation  
 WHERE realisation.mois LIKE'%$mois%' AND realisation.centre=4760 AND   realisation.parametre=1 AND realisation.anne='$ann' ";
 ...
 FROM previsions 
 WHERE previsions.mois LIKE'%$mois%' AND previsions.centre=4760 AND  previsions.parametre=2 AND previsions.anne='$ann' ";
        $resultp1 = mysqli_query($conn, $reqt9);
        $parametres2 = $resultp1->fetch_assoc();

        $reqt2 = "SELECT SUM(realisation) AS reali2
 FROM realisation  
 WHERE realisation.mois LIKE'%$mois%' AND realisation.centre=4760 AND   realisation.parametre=2 AND realisation.anne='$ann'";
 ...
 FROM previsions  
 WHERE previsions.mois LIKE'%$mois%' AND previsions.centre=4760 AND   previsions.parametre=4 AND previsions.anne='$ann' ";
        $resulttp1 = mysqli_query($conn, $reqt3);
        $parametres3 = $resulttp1->fetch_assoc();

        $reqt4 = "SELECT SUM(realisation) AS reali3
 FROM realisation  
 WHERE realisation.mois LIKE'%$mois%' AND realisation.centre=4760 AND   realisation.parametre=4 AND realisation.anne='$ann'";
 ...
 FROM previsions  
 WHERE previsions.mois LIKE'%$mois%' AND previsions.centre=4760 AND   previsions.parametre=3 AND previsions.anne='$ann' ";
        $resultim1 = mysqli_query($conn, $reqt5);
        $parametres4 = $resultim1->fetch_assoc();

        $reqt6 = "SELECT SUM(realisation) AS reali4
 FROM realisation  
 WHERE realisation.mois LIKE'%$mois%' AND realisation.centre=4760 AND   realisation.parametre=3 AND realisation.anne='$ann'";
        ...
        if (80 < $trc and $trc < 95) {
            $trc = 10 + (($trc - 80) * 0.6);
        }
        if ($trc == 95) {
            $prc = 20;
        }
        if (95 < $trc and $trc < 100) {
            $prc = 20 + (($trc - 95) * 1);
        }
        if ($trc == 100) {
            $prc = 25;
        }
        if (100 < $trc and $trc < 120) {
            $prc = 25 + (($trc - 100) * 0.5);
        }
        if ($trc == 120) {
            $prc = 35;
