...
<section class="home">
 ...
 FROM previsions  
 WHERE previsions.mois LIKE'%$mois%' AND previsions.centre=1161 AND   previsions.parametre=1 AND previsions.anne='$ann' ";
 ...
 FROM realisation  
 WHERE realisation.mois LIKE'%$mois%' AND realisation.centre=1161 AND   realisation.parametre=1 AND realisation.anne='$ann' ";
 ...
 FROM previsions 
 WHERE previsions.mois LIKE'%$mois%' AND previsions.centre=1161 AND  previsions.parametre=2 AND previsions.anne='$ann' ";
        $resultp1 = mysqli_query($conn, $reqt9);
        $parametres2 = $resultp1->fetch_assoc();

        $reqt2 = "SELECT SUM(realisation) AS reali2
 FROM realisation  
 WHERE realisation.mois LIKE'%$mois%' AND realisation.centre=1161 AND   realisation.parametre=2 AND realisation.anne='$ann'";
 ...
 FROM previsions  
 WHERE previsions.mois LIKE'%$mois%' AND previsions.centre=1161 AND   previsions.parametre=4 AND previsions.anne='$ann' ";
        $resulttp1 = mysqli_query($conn, $reqt3);
        $parametres3 = $resulttp1->fetch_assoc();

        $reqt4 = "SELECT SUM(realisation) AS reali3
 FROM realisation  
 WHERE realisation.mois LIKE'%$mois%' AND realisation.centre=1161 AND   realisation.parametre=4 AND realisation.anne='$ann'";
        ...
        <div class="det" style=" margin-top:7%">
            ...
            <table class="t1">
                <tr>
                    ...
                    <th> pondérations</th>
                    <th>Taux de rendement collectif <br> du : <?php echo $mois ?> , <?php echo $ann ?> </th>
                </tr>
                <tr>
                    <td>10</td>
                    <td>10</td>
